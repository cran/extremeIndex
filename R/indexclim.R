...
if ((score!="crps")&(score!="mae")){stop("score must be crps or mae")}
  ...
  if (compscore){
    ...
    score_clim=sapply(y[y>thresh[1]],function(i) mean(abs(y-i))+alpha)
    }
	if (score=="mae"){
    score_clim=sapply(y[y>thresh[1]],function(i) mean(abs(y-i)))
    }
...
if (estim_xi==TRUE){
pa=evir::gpd(y,threshold=yy,method="pwm")$par.ests[c(1,2)]
} else {
    pa=c(xi,evir::gpd(y,threshold=yy,method="pwm")$par.ests[2])
}
  ...
  result=list(quantiles=thresh,crps=score_clim,index=cvm,obs=y,xi=xi,score=score,estim_xi=estim_xi)
